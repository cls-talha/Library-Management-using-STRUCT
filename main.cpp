...
// < > prolly template hence to declare it we use this weird syntax
...
void erase_entity(vector<Book_Info> &book_arr,vector<Time> &time_arr,int &val,int &counter){
	cout<<"Enter which entity you want to Erase (1/"<<counter<<")"<<endl;
	cin >> val;
...
// custom ass function for sorting specific detail
		...
		if(choice==1){
			...
			/*	
				args:
					...
					time; same above shit but different hole (time)
					counter: how many entities user want to enter   
					wtf?? lab teacher made C++ low budget SQL
			*/
			...
			*/
			...
			/*	
				args:
					...
					compare_prices; custom ass function for comparion of specific shit {just sem shit diff hole}
...
// khoti ke bacho ne final year project banwa dia ha
